...
from bher.common.util import set_global_seeds
from bher.common.subproc_vec_env import SubprocVecEnv
from bher.common.dummy_vec_env import DummyVecEnv
from bher.common.wrappers import ClipActionsWrapper
from bher.common.tf_util import get_session
from bher.envs import register_envs
...
def make_env(env_id, env_type, mpi_rank=0, subrank=0, seed=None, reward_scale=1.0, gamestate=None, flatten_dict_observations=True, wrapper_kwargs=None, env_kwargs=None, logger_dir=None, initializer=None):
    ...
    if env_id.startswith('Fetch'):
        ...
        from bher.envs.multi_world_wrapper import SawyerGoalWrapper
        env = SawyerGoalWrapper(env)
        if not hasattr(env, '_max_episode_steps'):
            env = gym.wrappers.TimeLimit(env, max_episode_steps=100)
    elif env_id.startswith('Point'):
        from bher.envs.multi_world_wrapper import PointGoalWrapper
        env = gym.wrappers.TimeLimit(env, max_episode_steps=100)
        env = PointGoalWrapper(env)
    elif env_id.startswith('Reacher'):
        from bher.envs.multi_world_wrapper import ReacherGoalWrapper
        env._max_episode_steps = 100
